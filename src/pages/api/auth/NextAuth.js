    ...
    async signIn({ user, account, profile }) {
      if (account.provider === 'google') {
        try {
          ...
          const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://https://movie-streaming-v2-be.onrender.com'}/api/auth/google-login`, {
            email: profile.email,
            name: profile.name,
        ...
        try {
          ...
          const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://https://movie-streaming-v2-be.onrender.com'}/api/auth/facebook-login`, {
            email: profile.email,
            name: profile.name,
