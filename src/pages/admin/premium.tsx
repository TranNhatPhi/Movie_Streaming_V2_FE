...
const AdminPremiumPage: NextPageWithLayout<AdminPremiumPageProps> = () => {
  ...
  const fetchSubscriptions = async (tab: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://https://movie-streaming-v2-be.onrender.com${endpoint}${statusFilter}`, {
        headers: {
          'Authorization': `Bearer ${testToken}`
        }
      });
      ...
      if (isSuccess) {
        ...
        if (mappedSubscriptions.length > 0) {
          toast.success(`Đã tải ${mappedSubscriptions.length} đăng ký ${tab === 'pending' ? 'đang chờ duyệt' :
          ...
          toast.info(`Không có đăng ký nào ${tab === 'pending' ? 'đang chờ duyệt' :
            tab === 'approved' ? 'đã được duyệt' : 'đã bị từ chối'
            }`, {
            autoClose: 2000,
            hideProgressBar: true
